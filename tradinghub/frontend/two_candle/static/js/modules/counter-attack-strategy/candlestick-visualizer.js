...
export class CounterAttackCandlestickVisualizer extends CandlestickVisualizer {
    ...
    updateVisualization() {
        ...
        // Set color based on counter attack type
    ...
    /**
     * Render counter attack patterns on the chart
     * @param {Array} patterns - Array of counter attack pattern data
     */
    renderCounterAttackPatterns(patterns) {
        if (!patterns || patterns.length === 0) {
            console.log('No counter attack patterns to render');
            return;
        }

        console.log(`Rendering ${patterns.length} counter attack patterns`);
        ...
        patterns.forEach((pattern, index) => {
            this.renderCounterAttackPattern(pattern, index);
        });
    }

    /**
     * Render a single counter attack pattern
     * @param {Object} pattern - Pattern data
     * @param {number} index - Pattern index
     */
    renderCounterAttackPattern(pattern, index) {
        const { date, open, high, low, close, pattern_type } = pattern;
        
        // Create pattern marker
        const marker = document.createElement('div');
        marker.className = 'pattern-marker counter-attack-marker';
        ...
        marker.style.backgroundColor = pattern_type === 'bullish_counter_attack' ? '#28a745' : '#dc3545';
        marker.style.border = '2px solid white';
        marker.style.zIndex = '10';
        marker.title = `Counter Attack Pattern: ${date}`;
     ...
     */
    clearPatternMarkers() {
        const markers = document.querySelectorAll('.counter-attack-marker');
    ...
    initializeEventListeners() {
        ...
        rangeInputs.forEach(input => {
            ...
            input.addEventListener('input', () => {
                this.updateRangeValue(input);
                if (input.id === 'body_size_ratio' || input.id === 'close_tolerance') {
