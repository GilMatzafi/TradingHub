...
from .short_trade_executor import ShortTradeExecutor
from tradinghub.backend.shared.models.dto.trade_params import TradeParams

class ShortBaseBacktest:
    """Base class for all SHORT position pattern backtesters"""
    
    def __init__(self, pattern_detector: BasePattern):
        """
        Initialize the short position backtester
    ...
    def run_backtest(self, df: pd.DataFrame, pattern_params: Dict[str, Any], backtest_params: BacktestParams) -> Dict[str, Any]:
        """
        Run SHORT position backtest on historical data
        ...
        # Initialize SHORT trade executor
        ...
        trade_executor = ShortTradeExecutor(trade_params)
        ...
        for i in range(len(df) - backtest_params.entry_delay):
            ...
            if df.iloc[i][pattern_column]:
                # Enter SHORT position after entry_delay
