...
class EngulfingController:
    ...
    def analyze(self, data: Dict[str, Any]) -> Tuple[Any, int]:
        ...
        try:
            ...
            # Build engulfing-specific parameters
            pattern_params = PatternParams(
                body_size_ratio=float(data.get('body_size_ratio', 0.3)),
                lower_shadow_ratio=0.0,  # Not used for engulfing
                upper_shadow_ratio=0.0,  # Not used for engulfing
                ma_period=int(data.get('ma_period', 20)),
                require_green=False,  # Not used for engulfing
                require_high_volume=False  # Not used for engulfing
            )
            
            # Add engulfing-specific parameters
            engulfing_type = data.get('engulfing_type', 'both')
            require_trend = data.get('require_trend', True)
            pattern_params.engulfing_type = engulfing_type  # Add as custom attribute
