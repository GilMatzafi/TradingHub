"""
Three Inside Up Pattern Controller
Controller for handling Three Inside Up pattern analysis requests
"""
...
class ThreeInsideUpController:
    """Controller for Three Inside Up pattern analysis and backtesting"""
    
    def __init__(self):
        self.backtest_controller = BacktestController()
        self.stock_service = StockService()
        self.pattern_detector = ThreeInsideUpPattern()
    
    def analyze(self, data: Dict[str, Any]) -> Tuple[Any, int]:
        """
        Handle pattern analysis request for Three Inside Up patterns
        ...
        try:
            ...
            # Build Three Inside Up-specific parameters
            pattern_params = PatternParams(
                body_size_ratio=float(data.get('body_size_ratio', 0.6)),
                lower_shadow_ratio=0.0,  # Not used for Three Inside Up
                upper_shadow_ratio=0.0,  # Not used for Three Inside Up
                ma_period=int(data.get('ma_period', 20)),
                require_green=False,  # Not used for Three Inside Up
                require_high_volume=False  # Not used for Three Inside Up
            )
            
            # Add Three Inside Up-specific parameters
    ...
    def backtest(self, data: Dict[str, Any]) -> Tuple[Any, int]:
        """
        Handle backtest request for Three Inside Up patterns
